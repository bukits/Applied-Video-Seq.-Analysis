...
int main(int argc, char ** argv)
{
	...
	string dataset_path = "/home/avsa/AVSA2020datasets/dataset2012lite/dataset";
...
//
	//string dataset_path = "/home/avsa/Documents/AVSASlidesVideos_dataset"; //SET THIS DIRECTORY according to your download
...
// if (argc==3)	{input=argv[3]);}
	...
	string project_root_path = "/home/avsa/Documents/AVSA2020results/"; //SET THIS DIRECTORY according to your project
	string project_name = "Task5"; // project exe name
...
//			namedWindow("ForeGround Mask"); // final fgmask
			...
			double alpha_sh = 0.7;
			double beta_sh = 0.85;
			double saturation_th = 220;
			double hue_th = 160;
			int K = 5;
			int start_deviation = 100;


			fgseg::bgs avsa_bgs(tau, rgb, selective_bkg_update, alpha, threshold_ghosts2, alpha_sh, beta_sh, saturation_th, hue_th, K, start_deviation); //construct object of the bgs class
			...
			for (;;) {
				...
				// it=1 => initialize bkg
				 if (it==1)
					 ...
					 //avsa_bgs.initGaussianParams(img);
					 avsa_bgs.initMixturedGaussianParams(img);
					 //avsa_bgs.initCounter(img);
		        ...
		        //...
           		...
           		//avsa_bgs.removeGhosts();
		        //avsa_bgs.gaussian(img);
           		avsa_bgs.mixturedGaussian(img);
		        avsa_bgs.detectShadow();
