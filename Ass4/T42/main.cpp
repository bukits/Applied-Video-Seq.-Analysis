...
int main(int argc, char ** argv)
{
	...
	Feature *feature = new H_color();

	//Parameters for the candidate generation
	const int n_candidates = 4;
	const float step_s = 0.01;
	const int starting_frame_for_bb = 49;
	cv::Rect pred;
	int h, w;
	...
	for (int s=0; s<NumSeq; s++ )
	{
		...
		for (;;) {
			...
			if (frame_idx == 1) {
				...
				pred = gt_location;
				h = list_bbox_gt[starting_frame_for_bb].height;
				w = list_bbox_gt[starting_frame_for_bb].width;
