...
{
	std::vector<cv::Rect> cand_pred;
    int step_x = std::max((int)(prev_pred.width * step_s), 1);
    int step_y = std::max((int)(prev_pred.height * step_s), 1);
    for ( int i = -n_cand_pred; i <= n_cand_pred; i++)
    {
        for ( int j = -n_cand_pred; j <= n_cand_pred; j++)
        {
            int x_pos = prev_pred.x + ( i * step_x );
            int y_pos = prev_pred.y + ( j * step_y );

            if (x_pos >= 0 && //check if position is valid
            	y_pos >= 0 &&
            	x_pos + w <= frame.size().width &&
				y_pos + h <= frame.size().height)
            {
                cand_pred.push_back( Rect( x_pos, y_pos, w, h ) );
            }
        }
    }
    return cand_pred;
...
{
	cv::Rect pred = cand_pred[std::distance(dist.begin(),std::min_element(dist.begin(),dist.end()))];
	return pred;
}
