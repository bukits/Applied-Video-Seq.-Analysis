...
Kalman::Kalman(bool isConstantVelocity, int noMeasurement) {
	...
	std::vector<float> measurement_uncertainty_elements {125, 125};
...
Mat Kalman::paintBlobImage(cv::Mat frame, const ActualState &state, const cv::Point &meas) {
	Mat return_frame;
	frame.copyTo(return_frame);
	Scalar color;
	...
	rectangle(return_frame, p1_meas, p2_meas, Scalar(255, 255, 255), 2, 3, 0);
	rectangle(return_frame, p1, p2, color, 2, 3, 0);
	putText(return_frame, state.name, p1, FONT_HERSHEY_SIMPLEX, 1, color, 2);

	putText(return_frame, "Measurement: " + to_string(meas.x) + ";" + to_string(meas.y), Point(10, 15 + 20), FONT_HERSHEY_SIMPLEX, 1, Scalar(255, 255, 255), 2);
	if (state.name == "Corrected") {
		putText(return_frame, "Corrected: (" + to_string(state.point.x) + ";" + to_string(state.point.y) + ")", Point(10, 65 + 20), FONT_HERSHEY_SIMPLEX, 1, Scalar(255,0,0), 2);
	} else if (state.name == "Predicted") {
		putText(return_frame, "Prediction: (" + to_string(state.point.x) + ";" + to_string(state.point.y) + ")", Point(10, 65 + 20), FONT_HERSHEY_SIMPLEX, 1, Scalar(0,255,0), 2);
	}
	return return_frame;
}
...
Mat Kalman::paintTrackPath(const cv::Mat &frame) {
	...
	for (unsigned int i = 0; i < ground_truths.size(); ++i) {
		...
		drawMarker(track, gt, Scalar(255, 255, 255), MARKER_CROSS, 20, 2);
		drawMarker(track, actual, color, MARKER_SQUARE, 20, 2);
	}
	return track;
}
