...
Kalman::Kalman(bool isConstantVelocity, int noMeasurement) {
	...
	const float uncertainty_scaling = 10e5f;
	...
	uncertainty = uncertainty * uncertainty_scaling;
...
void Kalman::paintBlobImage(cv::Mat &frame, const ActualState &state, const cv::Point &meas) {
	...
	Point p1 = Point(state.point.x - 10, state.point.y - 10);
	Point p2 = Point(state.point.x + 10, state.point.y + 10);

	Point p1_meas = Point(meas.x - 10, meas.y - 10);
	Point p2_meas = Point(meas.x + 10, meas.y + 10);

	rectangle(frame, p1_meas, p2_meas, Scalar(255, 255, 255), 1, 8, 0);
	rectangle(frame, p1, p2, color, 1, 8, 0);
	putText(frame, state.name, p1, FONT_HERSHEY_SIMPLEX, 0.5, color);

	putText(frame, "Measurement: " + to_string(meas.x) + ";" + to_string(meas.y), Point(10, 15 + 20), FONT_HERSHEY_SIMPLEX, 1, Scalar(255, 255, 255), 1);
	if (state.name == "Corrected") {
		putText(frame, "Corrected: (" + to_string(state.point.x) + ";" + to_string(state.point.y) + ")", Point(10, 65 + 20), FONT_HERSHEY_SIMPLEX, 1, Scalar(255,0,0), 1);
	} else if (state.name == "Predicted") {
		putText(frame, "Prediction: (" + to_string(state.point.x) + ";" + to_string(state.point.y) + ")", Point(10, 65 + 20), FONT_HERSHEY_SIMPLEX, 1, Scalar(0,255,0), 1);
	}
}
...
Mat Kalman::paintTrackPath(const cv::Mat &frame) {
	...
	for (unsigned int i = 0; i < ground_truths.size(); ++i) {
		...
		drawMarker(track, gt, Scalar(255, 255, 255), MARKER_CROSS, 15, 2);
		drawMarker(track, actual, color, MARKER_SQUARE, 15, 2);
	}
	return track;
}
