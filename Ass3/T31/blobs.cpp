...
//returns true if there was at least one blob that is bigger than the minimum parameters.
//maxBlob will be the biggest blob.
bool biggestBlob(std::vector<cvBlob> bloblist_in, cvBlob &maxBlob, int min_width, int min_height)
{
	if (bloblist_in.size()==0) {return false;}
	//linear search for the max blob:
	maxBlob = bloblist_in[0];
	for(unsigned int i=1; i<bloblist_in.size(); i++)
	{
		if (bloblist_in[i].h*bloblist_in[i].w > maxBlob.h*maxBlob.w)
		{
			maxBlob=bloblist_in[i];
		}
	}
	if (maxBlob.h>=min_height and maxBlob.w>=min_width)
	{
		return true;
	}
	return false;
